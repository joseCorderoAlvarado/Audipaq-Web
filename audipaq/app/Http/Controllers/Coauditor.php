			...
			if (session()->has('s_tipoUsuario') ) 
			{
				...
				{   
					try
					{
						...
						return view('verListadoObservaciones_Coauditor',['listaObservaciones'=>$listaObservaciones,'id_acta'=>$id_acta,'listaPrioridad'=>$listaPrioridad, 'listaStatus'=>$listaStatus]);
					}
					catch (Exception $e) 
			        {
			        	\Session::flash('mensaje','Error al mostrar las observaciones, intentelo más tarde');
						return redirect('ver_Coauditorias');
			        }
				}
				elseif(session('s_tipoUsuario')=='4')
				{
				...
				elseif(session('s_tipoUsuario')=='3')
				{
					try
					{
						...
						if($Acta->save()){
							...
							\Session::flash('flash_message', '¡Acta agregada con éxito');
							return redirect('ver_Coauditorias');			
						}
						else {
							\Session::flash('mensaje','Error al agregar el acta');
							 return redirect('ver_Coauditorias');
						}
					}
					catch (Exception $e) 
			        {
			        	\Session::flash('mensaje','Error al agregar acta, intentelo más tarde');
						return redirect('ver_Coauditorias');
			        }
				}
				elseif(session('s_tipoUsuario')=='4')
